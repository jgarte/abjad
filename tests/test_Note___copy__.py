...
def test_Note___copy___04():
    ...
    note_2 = copy.copy(note_1)
    grace_container_2 = abjad.inspectx.after_grace_container(note_2)
...
def test_Note___copy___06():
    ...
    assert new_note is not note
    assert abjad.inspectx.parentage(note).parent is staff
    assert abjad.inspectx.parentage(new_note).parent is not staff
    assert isinstance(abjad.inspectx.parentage(new_note).parent, abjad.Staff)
    assert abjad.lilypond(new_note) == abjad.lilypond(note)
    assert abjad.lilypond(abjad.inspectx.parentage(note).parent) == abjad.lilypond(
        abjad.inspectx.parentage(new_note).parent
    )
