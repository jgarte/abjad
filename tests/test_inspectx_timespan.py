...
def test_inspectx_timespan_01():

    voice = abjad.Voice("c'8 d'8 e'8 f'8")
    for i, x in enumerate(voice):
        assert abjad.inspectx.timespan(x).start_offset == i * abjad.Offset(1, 8)


def test_inspectx_timespan_02():
...
def test_inspectx_timespan_07():

    tuplet = abjad.Tuplet((2, 3), "c'8 c'8 c'8")
    for i, x in enumerate(tuplet):
        assert abjad.inspectx.timespan(x).start_offset == i * abjad.Offset(1, 12)


def test_inspectx_timespan_08():
    ...
    for leaf, duration in zip(leaves, durations):
        assert abjad.inspectx.timespan(leaf).start_offset == offset
        offset += abjad.Offset(*duration)


def test_inspectx_timespan_09():
    """
...
def test_inspectx_timespan_16():
    ...
    for i, voice in enumerate(staff):
        start_offset = abjad.inspectx.timespan(voice).start_offset
        assert start_offset == i * abjad.Offset(4, 8)


def test_inspectx_timespan_17():
    """
    ...
    start_offset = abjad.inspectx.timespan(container[0]).start_offset
    assert start_offset == abjad.Offset(0)
    start_offset = abjad.inspectx.timespan(container[1]).start_offset
    assert start_offset == abjad.Offset(1, 2)


def test_inspectx_timespan_18():
    """
...
def test_inspectx_timespan_19():
    ...
    assert abjad.inspectx.timespan(voice[0]).start_offset == 0 * abjad.Offset(1, 4)
    assert abjad.inspectx.timespan(voice[1]).start_offset == 1 * abjad.Offset(1, 4)
    assert abjad.inspectx.timespan(voice[2]).start_offset == 2 * abjad.Offset(1, 4)


def test_inspectx_timespan_20():
    """
    ...
    tuplet_1 = abjad.Tuplet((2, 3), "c'8 c'8 c'8")
    voice = abjad.Voice([abjad.Note(0, (1, 8)), tuplet_1, abjad.Note(0, (1, 8))])
    assert abjad.inspectx.timespan(voice[0]).start_offset == 0 * abjad.Offset(1, 8)
    assert abjad.inspectx.timespan(voice[1]).start_offset == 1 * abjad.Offset(1, 8)
    assert abjad.inspectx.timespan(voice[2]).start_offset == 3 * abjad.Offset(1, 8)


def test_inspectx_timespan_21():
    """
    ...
    assert abjad.inspectx.timespan(tuplet[0]).start_offset == 0 * abjad.Offset(1, 6)
    assert abjad.inspectx.timespan(tuplet[1]).start_offset == 1 * abjad.Offset(1, 6)
    assert abjad.inspectx.timespan(tuplet[2]).start_offset == 2 * abjad.Offset(1, 6)


def test_inspectx_timespan_22():
    """
    ...
    assert abjad.inspectx.timespan(inner_voice).start_offset == abjad.Offset(2, 8)
    assert abjad.inspectx.timespan(outer_voice).start_offset == abjad.Offset(1, 8)


def test_inspectx_timespan_23():
    """
    ...
    staff = abjad.Staff([voice_1, voice_2])
    staff.simultaneous = True
    assert abjad.inspectx.timespan(staff[0]).start_offset == 0
    assert abjad.inspectx.timespan(staff[1]).start_offset == 0


def test_inspectx_timespan_24():
    """
    ...
    assert abjad.inspectx.timespan(voice_1).start_offset == 0
    assert abjad.inspectx.timespan(voice_2).start_offset == 0
    assert abjad.inspectx.timespan(voice_1b).start_offset == abjad.Offset(4, 8)
    assert abjad.inspectx.timespan(voice_2b).start_offset == abjad.Offset(4, 8)


def test_inspectx_timespan_25():
    """
    ...
    with pytest.raises(Exception):
        abjad.inspectx.timespan(staff[0], in_seconds=True).start_offset


def test_inspectx_timespan_26():
    """
    ...
    start_offset = abjad.inspectx.timespan(staff[0], in_seconds=True).start_offset
    assert start_offset == abjad.Offset(0)
    start_offset = abjad.inspectx.timespan(staff[1], in_seconds=True).start_offset
